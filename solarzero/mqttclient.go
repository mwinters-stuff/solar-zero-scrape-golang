...
func (mq *mqttClientImpl) WriteCurrentData(scrape SolarZeroScrape) {
	...
	mq.publish("dpowerflow", strconv.FormatInt(currentData.DPowerFlow, 10))
	mq.publish("export", strconv.FormatInt(currentData.Export, 10))
	mq.publish("import", strconv.FormatInt(currentData.Import, 10))
	mq.publish("load", strconv.FormatInt(currentData.Load, 10))
	mq.publish("solar", strconv.FormatInt(currentData.Solar, 10))
	mq.publish("soc", strconv.FormatInt(currentData.Soc, 10))
	mq.publish("charge", strconv.FormatInt(currentData.Charge, 10))
