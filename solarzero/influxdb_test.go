...
type influxLogHook struct {
	LastEvent *zerolog.Event
	LastLevel zerolog.Level
	LastMsg   string
}

func (h *influxLogHook) Run(e *zerolog.Event, l zerolog.Level, m string) {
	h.LastEvent = e
	h.LastLevel = l
	h.LastMsg = m
}
...
type InfluxDBWriterTestSuite struct {
	suite.Suite
	writer       solarzero.InfluxDBWriter
	mockWriteAPI *mocks.WriteAPI
	loghook      influxLogHook
	errCh        chan error
	currentData  jsontypes.CurrentData
	json         map[string]string
}

func (*InfluxDBWriterTestSuite) makeConfig() jsontypes.Configuration {
...
func (suite *InfluxDBWriterTestSuite) SetupTest() {
	suite.loghook = influxLogHook{}
	solarzero.Logger = log.Hook(&suite.loghook)

	suite.currentData = jsontypes.CurrentData{
		DeviceStatus:    1,
		DPowerFlow:      5,
		Export:          0,
		Import:          1026,
...
func (suite *InfluxDBWriterTestSuite) ConnectInflux() {
	...
	suite.writer = solarzero.NewInfluxDBWriter(&configuration)
...
func (suite *InfluxDBWriterTestSuite) TestErrorLog() {
	...
	assert.True(suite.T(), suite.writer.HasWriteError())
...
func (suite *InfluxDBWriterTestSuite) TestWriteDayData() {
	...
	daydata, err := jsontypes.UnmarshalDayData([]byte(suite.json["day"]))
...
func (suite *InfluxDBWriterTestSuite) TestWriteMonthData() {
	...
	monthdata, err := jsontypes.UnmarshalMonthData([]byte(suite.json["month"]))
	assert.Nil(suite.T(), err, "Err is not nil")
...
func (suite *InfluxDBWriterTestSuite) TestWriteYearData() {
	...
	yeardata, err := jsontypes.UnmarshalYearData([]byte(suite.json["year"]))
	assert.Nil(suite.T(), err, "Err is not nil")
